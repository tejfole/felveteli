...
Public Sub Publikalas_AzonositoPont_Sorszam_RangsorSzerint(Optional control As IRibbonControl)
    ...
    ' --- minimum pontszám szűrő (opcionális) ---
    ...
    minPontStr = LCase$(Trim$(InputBox( _
        ...
        "Hagyd üresen / írd be: mind / összes  › nincs szűrés.", _
        "Minimum pont", "mind")))
    ...
    If ixSzobeli = 0 Or ixIrasbeli = 0 Or ixBiz = 0 Then
        MsgBox "Hiányzó oszlop az elutasítás ellenőrzéséhez (szobeli/irasbeliossz/p_bizonyitvany).", vbCritical
        Exit Sub
    End If
...
NextRow:
    ...
    If nAcc = 0 And nRej = 0 Then
        MsgBox "Nincs találat a szűrés/min pont alapján.", vbExclamation
        Exit Sub
    End If
    ...
    msg = "Publikálható lista elkészült. Lap: " & wsOutput.Name & vbCrLf & _
          "Rangsorolt (sorszámozott): " & nAcc & " fő"
    If nRej > 0 Then msg = msg & vbCrLf & "Lista végén (Elutasítva, sorszám nélkül): " & nRej & " fő"
