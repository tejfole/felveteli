...
Public Sub AssignDatumNap_FromIdopontTabla( _
    ...
    If loT.ListRows.Count = 0 Or loT.DataBodyRange Is Nothing Then
        MsgBox "Nincs időpont a tbl_idopontok táblában. Vegyél fel időpontokat az Időpontok menüből.", vbExclamation
        Exit Sub
    End If
    ...
    If colIras > 0 Then
        ...
        If irasPont < KEVES_IRASBELI_KUSZOB Then
            ...
            Dim msg As String
            msg = "Kevés írásbeli pont (" & KEVES_IRASBELI_KUSZOB & " alatt) – alapból nem kap időpontot." & vbCrLf & vbCrLf & _
                  "Név: " & nev & vbCrLf & _
                  "Oktazon: " & okt & vbCrLf & _
                  "Írásbeli: " & irasPont & vbCrLf & vbCrLf & _
                  "Felülbírálod és mégis kiosztod az időpontot?"
    ...
    If activeDates.Count = 0 Then
        MsgBox "Nincs AKTÍV időpont a tbl_idopontok táblában.", vbExclamation
        Exit Sub
    End If

    ' ===== 3) lista: időpont + szabad hely =====
    ...
    If n = 0 Then
        MsgBox "Nincs szabad hely egyik aktív időpontban sem ennél a bizottságnál (" & biz & ").", vbExclamation
        Exit Sub
    End If
    ...
    Dim pick As Long
    pick = ChooseIndexFromList("Időpont választás (Bizottság " & biz & ")", items2)
    ...
    If CountAssignedInArr(arrD, biz, dtChosen, iBizD, iDtD) >= kapacitas Then
        MsgBox "Közben betelt ez az időpont. Válassz másikat.", vbExclamation
        Exit Sub
    End If
...
Fail:
    MsgBox "Hiba az időpont kiosztás közben: " & Err.Description, vbExclamation
End Sub
...
' FONTOS: legyen Public, hogy biztosan elérhető legyen minden modulból
