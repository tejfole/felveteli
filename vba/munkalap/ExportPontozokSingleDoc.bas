...
Sub ExportPontozok_Full_WithSummary()
    ...
    If MsgBox("Készítsek per-bizottság Word összesítő dokumentumot a sablon alapján (" & _
              fso.GetFileName(summaryTemplatePath) & ")?", vbYesNo + vbQuestion, "Összesítő") = vbYes Then
    ...
    If Not fso.FileExists(mainTemplatePath) Then
        MsgBox "A fő sablon nem található: " & mainTemplatePath, vbCritical
        Exit Sub
    End If
    On Error Resume Next
    ...
    If createSummaryDocs Then
        If Not fso.FileExists(summaryTemplatePath) Then
            MsgBox "Az összesítő sablon nem található: " & summaryTemplatePath & vbCrLf & "A summary dokumentumok nem készülnek.", vbExclamation
            createSummaryDocs = False
        Else
            ' másoljuk tempbe
